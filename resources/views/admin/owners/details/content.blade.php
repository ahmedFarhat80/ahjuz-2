<div class="flex-lg-row-fluid ms-lg-15">
  ...
  <ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-4 fw-bold mb-8">
    <!--begin:::Tab item-->
    <li class="nav-item">
      <a class="nav-link text-active-primary pb-4 active" data-bs-toggle="tab" href="#kt_owner_view_overview_tab">نظرة عامة</a>
    </li>
        ...
        <div class="menu-item px-5">
          <form method="POST" action="{{ route('admin.owners.destroy', $owner->id) }}">
            ...
            <div class="menu-item px-3">
              <button onclick="destroy(event)" type="submit" class="btn btn-link text-danger">حذف المالك</button>          
            </div>
          </form>
        </div>
        <!--end::Menu item-->
            ...
            <!--begin::Table head-->
              <thead class="border-bottom border-gray-200 fs-7 fw-bolder">
                ...
                <tr class="text-start text-muted text-uppercase gs-0" role="row">
                  <th class="min-w-100px sorting" tabindex="0" rowspan="1" colspan="1" style="width: 117.125px;">اسم الوحدة</th>
                  <th class="sorting" tabindex="0" rowspan="1" colspan="1" style="width: 71.9px;">جاهز للعرض</th>
                  <th class="sorting" tabindex="0"rowspan="1" colspan="1"style="width: 66.825px;">الحالة</th>
                </tr>
                <!--end::Table row-->
              </thead>
              <!--end::Table head-->
              ...
              @foreach ($properties as $property)
                <tr>
                    <td>
                      <a href="{{ route('admin.properties.show', $property->id) }}">{{ $property->name }}</a>
                    </td>
                    <td>
                      <span class="badge badge-light-{{ $property->ActiveClass }}">{{ $property->is_active->description }}</span>
                    </td>
                    <td>
                      <span class="badge badge-light-{{ $property->statusClass }}">{{ $property->status->description }}</span>
                    </td>
                </tr>
              @endforeach
