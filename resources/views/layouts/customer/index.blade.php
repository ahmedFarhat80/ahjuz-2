...
<body>
    ...
    <script>
      ...
      function setDates(startDate, endDate) {
        ...
        switch (true) {
            case (days == 1): daysText = ' يوم واحد'; break;
            case (days == 2): daysText = ' يومان'; break;
            case (days > 2 && days < 11): daysText = days + ' أيام'; break;
            case (days > 10): daysText = days + ' يوماً'; break;
        }

        $('#kt_daterangepicker_1').text(startDate + ' - ' + endDate)
        $('#days').text(daysText);
    ...
    <script>
        function destroy(event) {
            ...
            if (!form.length) {
                form = $(event.target).parent().siblings().find('form');
            }
    ...
    <script>
        ...
        var channel = pusher.subscribe('private-App.Models.Customer.' + {{ auth_customer()->id }});
        ...
        channel.bind('Illuminate\\Notifications\\Events\\BroadcastNotificationCreated', function(data) {
            ...
            switch (data.type) {
                case 'BookingIsCanceled':
                    return fireNotification('تم إلغاء حجز لديك', '/home')
                    break;
                case 'NewMessage':
                    return fireMessageNotification('يوجد لديك رسالة جديدة', `/messages/${data.property_id}/{{ auth_customer()->id }}`, data.name, data.message)
                    break;
                default:
                    break;
            }
        ...
        function fireMessageNotification(message, route, name, body) {
            ...
            $('#ul_messages_notifications').prepend(`
                <li style="background-color: #eee">
                    <a class="dropdown-item" href="${route}" style="white-space: normal;">
                        <strong> يوجد لديك رسالة من ${name}.</strong>
                        <span>${body}</span>
                        <div class="text-muted">منذ لحظات</div>
                    </a>
                </li>
