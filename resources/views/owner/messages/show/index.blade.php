  ...
  <div class="messages mt-5">
    ...
    <div class="row">
      <div class="col-lg-8">
        ...
        <form id="add_message" action="{{ route('owner.messages.store', $conversation->id) }}" method="POST" enctype="multipart/form-data"> @csrf
          ...
          <div class="p-3 fw-bold border-bottom">
            ...
            <ul class="p-0 mt-3">
              <li class="d-flex justify-content-between">
                <span class="text-muted">اسم الزبون:</span>
                <span>{{ $conversation->customer->full_name }}</span>
              </li>
              <li class="d-flex justify-content-between">
                <span class="text-muted">رقم الهاتف:</span>
                <span dir="ltr">{{ country_code($conversation->customer->phone) }}</span>
              </li>
              <li class="d-flex justify-content-between">
                <span class="text-muted">البريد الالكتروني:</span>
                <span dir="ltr">{{ $conversation->customer->email }}</span>
              </li>
            </ul>
          </div>
        </div>
